...
const ScanScreen = ({ navigation }) => {
  ...
  const SERVER_URL = 'http://10.28.191.54:5000';
...
const handleScan = async () => {
  ...
  try {
    ...
    if (videos.length > 0) {
      ...
      const response = await fetch(`${SERVER_URL}/analyze-video`, {
        method: 'POST',
        body: formData,
        headers: {
          Accept: 'application/json',
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerText}>Vehicle Incpection</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.previewColumn}>
        ...
        {annotatedImage && <Image source={{ uri: annotatedImage }} style={styles.imageColumn} />}
